...
public class SubmitServiceImpl implements ISubmitService {
  ...
  private RetBean invokeWithRMI(ReqBean reqBean) {
    ...
    String msg = null;
    try {
      msg = lookup.submit(reqBean.getWorkType(), reqBean.getId(), reqBean.getTime(), reqBean.getAccount(),
              reqBean.getPwd());
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  private RetBean invokeWithJMX(ReqBean reqBean) throws Exception {
    ...
    AppCtrl appCtrl = JmxClientTools.makeRMIProxy(work.getJmxIp(), work.getJmxPort(), "sun$rmi", "jmx:name" + "=appCtrl", AppCtrl.class, pMap);
    try {
